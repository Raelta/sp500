    ...
    with st.expander("⚠️ Data Quality Issues Detected", expanded=False):
        ...
        with tab2:
            ...
            if count > 0:
                ...
                st.dataframe(val_report['missing_values']['data'], use_container_width=True)
        ...
        with tab3:
            ...
            if count > 0:
                ...
                st.dataframe(val_report['intraday_gaps']['data'], use_container_width=True)
...
with st.sidebar.form("params_form"):
    run_btn_top = st.form_submit_button("Run Analysis", use_container_width=True, key="run_top")
    
    st.header("Bump Parameters")
    bump_len = st.slider("Bump Length (min)", 3, 20, 5, help="Duration of the initial trend window in minutes.")
    bump_thresh_type = st.radio("Bump Threshold Type", ["percent", "value"], index=0, help="Choose 'percent' for relative change (%) or 'value' for absolute price difference.")
    ...
    slide_thresh_type = st.radio("Slide Threshold Type", ["percent", "value"], index=0, help="Choose 'percent' for relative change (%) or 'value' for absolute price difference.")
    ...
    days = st.multiselect("Days", 
        ...
        help="Select which days of the week to include in the analysis."
    )
    
    run_btn_bottom = st.form_submit_button("Run Analysis", use_container_width=True, key="run_bottom")

# State management for results
if 'results' not in st.session_state:
    st.session_state.results = None

if run_btn_top or run_btn_bottom:
