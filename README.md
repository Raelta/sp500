...
4.  **Overlap Filtering**: A post-processing step ensures that reported matches do not overlap in time. If multiple matches occur within overlapping windows, the one with the highest slide magnitude is preserved.
...
### 2. Utility Tests (`tests/test_bugfixes.py`)
These tests verify helper functions and utility logic.

| Test Function | Purpose | Methodology |
| :--- | :--- | :--- |
| `test_derive_result_blob_name` | Verify blob name parsing. | Check various input strings to ensure result filenames are derived correctly from metadata filenames. |
...
### 4. Integration Tests (`tests/test_app.py`)
...
### 5. Goal Seek Tests (`tests/test_goal_seek.py`)
...
You can verify if the cloud job is running correctly by checking the logs:
